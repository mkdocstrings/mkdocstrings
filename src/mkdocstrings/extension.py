...
class AutoDocProcessor(BlockProcessor):
    ...
    def _process_headings(self, handler: BaseHandler, element: Element) -> None:
        ...
        for heading in headings:
            ...
            if "--" in rendered_id:
                title = f"<code>{rendered_id.split('--', 1)[0]}</code> — {heading.text}"
            else:
                title = f"<code>{rendered_id}</code>"
            self._autorefs.register_anchor(rendered_id, title=title, primary=True)
            ...
            for alias in aliases:
                if alias != rendered_id:
                    self._autorefs.register_anchor(alias, rendered_id, primary=False)
            ...
            if "data-role" in heading.attrib:
                self._handlers.inventory.register(
                    ...
                    uri=f"{page.url}#{rendered_id}",  # type: ignore[attr-defined]
                )
                for alias in aliases:
