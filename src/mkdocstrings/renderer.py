...
class HTMLRenderer:
    ...
    def render_object(self, obj, heading_level, elem: etree.Element):
        ...
        if show_top_object_heading and (not self.hide_no_doc or obj.has_contents()):
            signature = self.render_signature(obj)
            module_heading = f"{obj.path if show_top_object_full_path else obj.name}{signature}"
            module_permalink = obj.path
            module_toc_title = obj.name
            if obj.is_method or obj.is_function:
                module_toc_title += "()"
            ...
            elem_heading.set("data-toc-label", module_toc_title)
            elem_heading.set("id", module_permalink)
            elem_heading.text = CodeHilite(src=f"{module_heading}", lang="python").hilite()
    ...
    @staticmethod
    def render_docstring(obj, elem: etree.Element):
        for section in obj.docstring.blocks:
            if section.type == Section.Type.MARKDOWN:
                lines.extend(section.value)
            elif section.type == Section.Type.PARAMETERS:
